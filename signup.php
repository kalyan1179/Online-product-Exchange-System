...
session_start();
		...
		if(isset($_POST['submit'])){
			...
			$name = $_POST['name'];$_SESSION['x']=$name;
			$username = $_POST['username']; $_SESSION['y']=$username;
			...
			if($password==$confirmpassword){
				...
				// }
					$q1 = "INSERT INTO `user`( `name`, `username`, `email`, `phone no`, `city`, `state`, `gender`, `address`, `password`,`otp`,`valid`) VALUES ('$name','$username','$email','$ph','$city','$state','$gender','$address','$password','$otp',$v)";
					// $q1 = "INSERT INTO `user`(`uid`, `name`, `username`, `email`, `phone no`, `city`, `state`, `gender`, `address`, `password`, `otp`, `valid`) VALUES ([value-1],[value-2],[value-3],[value-4],[value-5],[value-6],[value-7],[value-8],[value-9],[value-10],[value-11],[value-12])";
				...
				<script type="text/javascript">
					...
					window.location.href="PHPMailer/index.php";
...
<!-- <div class="tt"></div> -->
  <div class="signin">
  	<p>
  		<h1 style="color:seagreen;font-family:times new roman;font-style:italic;font-weight: bold;font-stretch:ultra-expanded;text-align: center;margin: 20px 0 20px 0;font-size: 45px;">Signup here!!</h1>
	...
	<form id="frm1" action="" method="POST" ng-app="myApp" ng-controller="validateCtrl" name="myForm" novalidate>
		<label>Name:  </label> 	
		<input style="width: 50%;margin-left: 100px;" type="text" name="name" placeholder="Enter your Name" required></input>
		<br><br>
		<label>UserName:  </label> 	
		<input onchange="existing_user()" style="width: 50%;margin-left: 60px;" type="text" name="username" placeholder="Enter User Name" minlength="4" ng-model="username"  ng-pattern="/^[a-zA-Z0-9]{4,}$/" id="username" required>
      ...
      model = <code>{{model}}</code> -->
		...
		<input style="width: 50%;margin-left: 80px;" type="email" name="email" placeholder="Enter Mail-id" required>
		<br><br>
		<label>Gender:</label>
		...
		<input style="width: 50%;margin-left: 50px;" type="tel" name="phone" pattern="[6-9]{1}[0-9]{9}" placeholder="Enter phone number" required>
		<br><br>
		<div style="width: 20%;float: left;"><label>Address:</label></div>		
		<!-- <br> -->
		<textarea rows="4" cols="27" maxlength="255" name="address" style="margin-right: 120px; border-radius: 10px;padding: 10px;width: 280px;font-size: 100%;margin-top: 5px;float: right;"></textarea><br>
		<p style="font-size: 12px;margin-bottom: -30px;color: yellow;margin-left: 160px;">**Max of 255 charecters**</p>
		<br><br>
		<label>City:</label>
		<input style="width: 50%;margin-left: 115px;" type="text" name="city" placeholder="Enter city" required>
		<br><br>
		<label>State:</label>
		<input style="width: 50%;margin-left: 105px;" type="text" name="state" placeholder="Enter state" required>
		<br><br>	
		
		<label for="password">Password:</label>
       <input style="width: 50%; margin-left: 65px;" type="password" id="password" name="password" ng-model="formData.password" ng-minlength="8" ng-maxlength="20" ng-pattern="/(?=.{8,20}$)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z])/" placeholder="Enter Password" required  /><br>
   ...
   <span style="color: red"  ng-show="!myForm.password.$error.required && !myForm.password.$error.minlength && !myForm.password.$error.maxlength && myForm.password.$error.pattern && myForm.password.$dirty">Must contain one lower &amp; uppercase letter, and one non-alpha character (a number or a symbol.)</span>
       ...
       <input style="width: 45%;margin-left: 14px;" required=""  type="password" id="password_c" placeholder="Re-Enter Password" name="password_c" ng-pattern="/(?=.{8,20}$)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z])/" ng-model="formData.password_c" valid-password-c   /><br>
   ...
   <span style="color: green;"  ng-show="!myForm.password_c.$error.required && !myForm.password_c.$error.noMatch && myForm.password.$dirty">Passwords matched.</span><br><br>
		...
		box-shadow:0px 0px 5px 5px seagreen;color: white;"ng-show="!myForm.username.$error.pattern && !myForm.password_c.$error.pattern && !myForm.password.$error.pattern && !myForm.password_c.$error.noMatch" type="submit" name="submit" id="i">SignUp</button>
	</form>
  </div>
</body>
</html>
