...
*/
		...
		.divbox{
			float: left;
			width: 280px;height: 350px;background: white; margin-top: 30px;margin-bottom: 10px;margin-left: 45px;border-radius: 10px;/*border: 2px solid rgb(50,50,50);
			/*padding: 0 12px;*/
		}
		div {
	...
	<div class="nav" >
		...
		<?php
			while ($row = mysqli_fetch_assoc($r)) {
				// echo $row['pname'];
				// echo $str;
				// $position = strpos($str, $row['pname'],0); 
				// echo "$position";
				if (preg_match("/$str/i", $row['pname'])) {
