<?php require 'connect.inc.php'; 
	...
	if(isset($_SESSION['id'])){
		$dgh = $_SESSION['id'];
		$q = "SELECT * FROM `product` where pid in (select wpid from `wishlist` where wuid = '$dgh' )";
		$rb = mysqli_query($conn,$q);
	?>
<!DOCTYPE html>
<html>
<head>
...
*/
		...
		.divbox{
			float: left;
			width: 280px;height: 395px;background: white; margin-top: 0px;margin-bottom: 10px;margin-left: 45px;border-radius: 10px;/*border: 2px solid rgb(50,50,50);
			/*padding: 0 12px;*/
			text-decoration: none;
		}
		div {
...
<body style="width: 100%;">
	...
	<p style="text-align: center;
			font-size: 50px;
			color:rgb(0,0,0);
			font-family: cursive;">My Wishlist</p>
		...
		<a href="product.php?ids=<?php echo "".$cv; ?>" style="color: black;text-decoration: none;">
	<div class="divbox">
		...
		<div style="width: 280px;">
			...
			<div style="width: 180px;float: left;height: 50px; padding: 5px;margin-left: 0px;font-size: 14px; "><?php echo $row1['city'].",".$row1['state'];?>
</div>
<div style="float: right;width: 100px;padding: 5px;">
		...
		<?php
			if($row['sold']==1){
				?>
					<a href="k.php?id=<?=$row['pid']?>"><button style="background: rgba(10,10,0,1); width: 100%;border-radius: 0px;border: 0 ;bottom: 0;font-size: 20px;font-weight: bold;color: red">THIS IS SOLD(REMOVE)</button></a>
				<?php
			}else{
				?>
				<a href="k.php?id=<?=$row['pid']?>"><button style="background: rgba(10,10,0,1); width: 100%;border-radius: 0px;border: 0 ;bottom: 0;font-size: 20px;font-weight: bold;color: red">REMOVE</button></a>
				<?php
			}
		?>			
